...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        try:
            with socket_lock:
                ...
                self.transport.send(self.create_presence())
                self.process_server_ans(self.transport)
    ...
    def process_server_ans(self, client):
        logger.debug(f"Разбор сообщения от сервера: {message}")
        data = client.recv(2048)
        package = read_package(data)
        if package:
            print(package)
            if package["action"] == "message":
                sender = package["sender"]
                data = package["message"]
                logger.debug(f"Получено сообщение от пользователя {sender}:{data}")
                self.database.save_message(sender, "in", data)
    ...
    def contacts_list_update(self):
        ...
        logger.debug(f"Сформирован запрос {msg}")
        with socket_lock:
            self.transport.send(message(msg))
            ans = self.process_server_ans(self.transport)
            for contact in ans["message"]:
